    ...
    <div class="container">
        <div class="col-md-12">
            <div class="wrapper overflow-hidden">
                <form id="order" class="form" action="" enctype="multipart/form-data" method="post" accept-charset="utf-8" name='info-order' novalidate>
                   <?php
                            ...
                            <?php if($this->session->userdata('cart')):
                                ...
                                foreach ($data as $key => $value) :
                                    ...
                                    <tr>
                                        ...
                                        <td id="nameproduct"><?php echo $row['name']; ?></td>
                                        <td><input disabled style="width: 50px;" type="text" name="" id="number_by" name="numberBy" class="text-center" value="<?php echo $value ?>"></td>
                                        <td id="price">
                                            <?php
                                            if($row['price_sale'] > 0){
...
<script>
    ...
    function sendSMS(){
            ...
            var number_by = $("#number_by").text();
            var message = `Đơn hàng ${name} có giá ${price} và số lượng mua là ${number_by} đang được vận chuyển`;
            ...
            fetch(" https://smsnhom7-app.herokuapp.comdisable/api/v1/sms", requestOptions)
              .then(response => response.text())
              .then(result => console.log(result))
              .catch(error => console.log('error', error));
    }
    // Check count xem so luong co hop ly hay khong
    function checkCount(){
            var myHeaders = new Headers();
            myHeaders.append('Content-Type', 'application/json');
            myHeaders.append('Accept', 'application/json');
            var raw = JSON.stringify({
                  "id":parseInt($("#productId").text()),
                  "numberBy": parseInt($("#number_by").text()),
                });
            var requestOptions = {
                  method: 'POST',
                  headers: myHeaders,
            ...
            fetch("https://newservice-app.herokuapp.com/nhom7/product/checkNumberBy", requestOptions)
      ...
      // {
      //       if(result != null){
      //           if((result.numberBuy + numberJustBuy) - result.number < 0){
      //               alert("Đừng làm thế");
