...
// Ở đây là để đăng nhập, đăng ký
// View của đăng nhập đăng ký: views/module/topbar
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        $this->load->library('form_validation'); // Load library form_validate
        $this->form_validation->set_rules('username', 'Tài khoản', 'required|min_length[6]|max_length[32]'); // Three argrument
        $this->form_validation->set_rules('password', 'Mật khẩu', 'required|min_length[6]|max_length[32]');
        if($this->form_validation->run() ==TRUE){
            $username = $_POST['username'];
            $password = md5($_POST['password']); // MD5: encode password and compare with password in Database
            ...
            $this->data['title']='Triều Khúc Store - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            // $this->email->from('sale.smart.store.2019@gmail.com', 'Triều Khúc Store');
            // $this->email->to($email);
            // $this->email->subject('Hệ thống Triều Khúc Store - Quà thành viên mới');
            // $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Triều Khúc, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
            //     Hãy sử dụng tài khoản để mua hàng để tích lũy nhận thêm nhiều ưu đãi !!!!');
            // $this->email->send();
             $this->data['success']='Đăng ký thành công!';

        }  
        $this->data['title']='Triều Khúc store - Đăng ký tài khoản';   
    ...
    function check_mail(){ 
        $email = $this->input->post('email'); // Receive name email and 
